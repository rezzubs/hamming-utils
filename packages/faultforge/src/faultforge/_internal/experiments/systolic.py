...
See `faultforge.experiments.systolic` for a general overview.

NOTE: `ReliabilityMetric`/`compute_score`/`BatchReliability`/the `_batch_*`
scoring helpers are duplicated here from
`faultforge._internal.experiments.encoded_memory` rather than shared,
because that module is currently being refactored elsewhere. Once that
refactor lands, fold this copy back into a shared module.
"""
...
from faultforge._internal.experiment import Experiment, ExperimentDisplay, StopCondition
...
from faultforge._rust.systolic import ArrayConfig, Fault
...
class SystolicFaultInjection(Experiment):
    ...
    `EncodedFaultInjection`): each run draws one fault id from a `Picker`
    over `fault.radix(array)`, applies it via `backend.set_fault`, and scores
    one inference pass over the dataset. How many runs happen is controlled
    entirely by stop conditions.
    """
